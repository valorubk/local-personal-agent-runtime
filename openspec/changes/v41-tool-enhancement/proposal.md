...
- 增加读取操作系统配置的本地工具，用于返回平台、系统版本、CPU 架构、用户目录、当前目录、Shell、语言区域和环境变量摘要等非敏感信息。
- 增加打开操作系统 App 的本地工具，V1 仅支持 macOS，通过 App 名称请求系统打开已安装应用。
- 增加发送 HTTP 请求并解析响应的本地工具，支持常见 HTTP 方法、headers、body、timeout，并对 JSON 响应自动解析为结构化摘要。
