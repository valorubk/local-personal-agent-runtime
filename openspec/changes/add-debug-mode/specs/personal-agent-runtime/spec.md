...
- **AND** 记录包含 LLM 输入、LLM 输出、LLM 模型相关信息、`session_id`、`trace_id` 和系统时间

#### Scenario: 持久化工具调用记录
- **WHEN** 调试模式下一轮对话记录 Tool 调用
- **THEN** 系统在当天调试 SQLite 文件中保存工具调用类型记录
- **AND** 记录包含 Tool 名称、输入、输出或错误、`session_id`、`trace_id` 和系统时间

#### Scenario: 持久化 Skill 调用记录
- **WHEN** 调试模式下一轮对话记录 Skill 调用
- **THEN** 系统在当天调试 SQLite 文件中保存 Skill 调用类型记录
- **AND** 记录包含 Skill 名称、输入、输出或错误、`session_id`、`trace_id` 和系统时间
