...
- 通过 YAML 文件声明 stdio 或 Streamable HTTP MCP Server，并在 Babyface Session 启动时加载。
...
新增 `personal_agent/mcp/config.py` 解析 YAML，并用 Pydantic 或 dataclass 做字段校验：server 名称、`enabled`、`transport`、`command`、`args`、`url`、`headers`、`env`、`timeout_seconds`。配置错误抛出 `ConfigError`，CLI 继续以中文友好错误展示。
...
- Streamable HTTP MCP Server 可能需要鉴权或 session 头 → YAML 支持 headers 配置，客户端层负责保存和复用 MCP session 状态，错误时只展示摘要，不把敏感 header 打印到 CLI。
...
4. 更新 README，给出 stdio 和 Streamable HTTP 的最小 YAML 示例和配置路径说明，并明确本次不提供通用 HTTP Tool。
