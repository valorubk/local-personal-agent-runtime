...
- 通过 YAML 文件声明 stdio MCP Server，并在 Babyface Session 启动时加载。
...
- 保持 Memory SQLite 默认位置 `.babyface/memory/memory.sqlite3` 不变；MCP YAML 配置使用独立路径。

**Non-Goals:**

- 不实现 HTTP、SSE 或远程 MCP 传输，V1 只支持本地 stdio MCP Server。
- 不提供前端、交互式配置向导、远程 MCP 市场或插件安装器。
- 不实现细粒度权限策略、每次 MCP Tool 调用前确认、审计后台或 Multi-Agent。
- 不改变现有内置 Tool 的 OpenAI schema 与行为。

## Decisions

### Decision: MCP 配置使用独立 YAML 文件

在 `Settings` 中新增 `mcp_config_path: Path | None`，读取优先级为：`BABYFACE_MCP_CONFIG_PATH` 环境变量、TOML 运行配置中的 `mcp_config_path`、项目默认 `.babyface/config/mcp.yaml`。如果默认文件不存在，则视为未配置 MCP，不报错。
...
选择 YAML 是因为 MCP Server 往往有列表、嵌套 env 和参数数组，YAML 比 TOML 更适合用户手写。备选方案是把 MCP 配置塞进 `babyface.toml`，但这会让 LLM/API key 等运行配置与外部工具清单混在一起，也不利于未来单独分享 MCP 配置。

### Decision: 新增 MCP 配置解析与客户端边界

新增 `personal_agent/mcp/config.py` 解析 YAML，并用 Pydantic 或 dataclass 做字段校验：server 名称、`enabled`、`transport`、`command`、`args`、`env`、`timeout_seconds`。配置错误抛出 `ConfigError`，CLI 继续以中文友好错误展示。

新增 `personal_agent/mcp/client.py` 管理 stdio MCP Server 的生命周期：启动子进程、初始化会话、列出 Tool、执行 Tool、关闭连接。该模块是唯一直接依赖 MCP SDK 的地方，降低外部协议变化对 Runtime 的影响。
...
备选方案是自动重命名冲突 Tool，但 LLM 看到的工具名称会变得不可预测，不利于用户调试 YAML 配置。
...
如果某个 MCP Server 启动失败，管理器记录启动错误并跳过该 Server；只要仍有可用内置 Tool 或其他 MCP Tool，Session 继续运行，并在启动阶段向用户展示降级提示。

## Risks / Trade-offs

- MCP Server 可能执行高风险本地操作 → V1 通过用户显式 YAML 配置、默认不启用和清晰来源展示降低误用风险；细粒度权限放到后续迭代。
...
- stdio 子进程可能挂起 → 每个 Server 和 Tool 调用使用 `timeout_seconds`，Session 退出时统一关闭连接。
- YAML 增加新依赖 → 优先选择维护良好的轻量 YAML 解析库，并用配置解析单元测试锁定错误信息。

## Migration Plan

1. 增加依赖与配置字段后，默认行为保持不变：没有 MCP YAML 文件时只加载内置 Tool。
2. 新增 MCP 配置解析、Tool 适配器和管理器，并用 fake MCP client 覆盖单元测试。
3. 在 CLI Session 构造 Runtime 前接入 MCP 管理器，确保异常降级和退出关闭都可测试。
4. 更新 README，给出最小 YAML 示例和配置路径说明。
5. 回滚时删除 MCP YAML 配置或取消 `BABYFACE_MCP_CONFIG_PATH` 即可恢复到内置 Tool 模式。
