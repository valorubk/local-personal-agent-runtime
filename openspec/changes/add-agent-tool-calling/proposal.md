...
- 新增 YAML 配置文件能力，用于声明一个或多个外部 MCP Server，包括 server 名称、启动方式、环境变量、启用状态和超时等基础字段。
...
- V1 目标聚焦本地 CLI 中的 MCP stdio Server 接入；非目标包括 HTTP API、远程托管 MCP 管理平台、前端配置页面、复杂权限策略、Scheduler 和 Multi-Agent。
...
- 影响依赖：可能需要增加 YAML 解析库和 MCP 客户端相关依赖。
- 影响测试：需要覆盖 YAML 配置解析、MCP Server 生命周期、Tool Registry、Tool 调用成功/失败、配置错误和 CLI 降级行为。
